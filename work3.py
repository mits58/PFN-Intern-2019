...
for i, B in enumerate(Batch_Size):
  ...
  ax_SGD[0,0].plot(np.array(range(len(train_loss_SGD))), train_loss_SGD, color = colorlist[i], label = '$B={0}$'.format(B))
  ax_SGD[0,1].plot(np.array(range(len(train_acc_SGD))), train_acc_SGD, color = colorlist[i], label = '$B={0}$'.format(B))
  ax_SGD[1,0].plot(np.array(range(len(test_loss_SGD))), test_loss_SGD, color = colorlist[i], label = '$B={0}$'.format(B))
  ax_SGD[1,1].plot(np.array(range(len(test_acc_SGD))), test_acc_SGD, color = colorlist[i], label = '$B={0}$'.format(B))

  # Momentum SGDを用いる分類器に関して同様のことを行う
  ax_MSGD[0,0].plot(np.array(range(len(train_loss_MSGD))), train_loss_MSGD, color = colorlist[i], label = '$B={0}$'.format(B))
  ax_MSGD[0,1].plot(np.array(range(len(train_acc_MSGD))), train_acc_MSGD, color = colorlist[i], label = '$B={0}$'.format(B))
  ax_MSGD[1,0].plot(np.array(range(len(test_loss_MSGD))), test_loss_MSGD, color = colorlist[i], label = '$B={0}$'.format(B))
  ax_MSGD[1,1].plot(np.array(range(len(test_acc_MSGD))), test_acc_MSGD, color = colorlist[i], label = '$B={0}$'.format(B))
  # 作成した分類器を削除する
  del clf_SGD
  del clf_MSGD
